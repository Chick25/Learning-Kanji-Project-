...
function updateUserStreak(user) {
  ...
  if (diffDays === 1) {
    user.streak += 1;        // học liên tục
  } else if (diffDays > 1) {
    user.streak = 1;         // đứt streak
  }
  // diffDays === 0 → cùng ngày → không tăng
...
function loadUser() {
  ...
  if (data) {
    Object.assign(CURRENT_USER, JSON.parse(data));
  }
}
...
function showResult(){
  ...
  /* ===== 1. CẬP NHẬT STREAK USER ===== */
  updateUserStreak(CURRENT_USER);
  saveUser();
  renderStreak();
...
function renderStreak() {
  ...
  if (!el) return;
  el.innerText = CURRENT_USER.streak;
}
