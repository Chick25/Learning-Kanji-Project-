...
function endGame() {
  ...
  let winner = "";
  if (playerScore > botScore) winner = "🎉 BẠN THẮNG! I'll comeback, human! ☠️";
  else if (botScore > playerScore) winner = "🤖 BOT THẮNG! You are chickenn! 🍗";
...
function checkWrite() {
  ...
  for (let p of userStroke) {
    if (tempCtx.isPointInStroke(new Path2D(path), p.x / 2, p.y / 2)) matchCount++;
  }
  ...
  if (ratio >= 0.35) {
    ...
    resultDiv.innerHTML = `<span class="wrong">SAI NÉT LẦN ${failCount+1}</span>`;
    failCount++;
    if(failCount===3){
        playerScore-=0.5;
        drawFullAnswer();
